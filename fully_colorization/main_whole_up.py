...
parser.add_argument("--test_dir", default='./data/test/frame2', type=str, help="test dir path")
...
def Bilateral_NN(color_image, neigh, knn_k):
    ...
    idxs = neigh.kneighbors(np.reshape(color_image_all, [-1, 5]), knn_k, return_distance=False)
#    idxs = np.zeros([h*w, 5],dtype="int32")
    return idxs

# frame t, t+1 pair 만들기
...
#     objDict["warped"] = tfa.image.dense_image_warp(C0, input_flow_backward[:,:,:,0:2], name="op_warp")
    ...
    #-------------RefineNet---------------#
    # 0 -> 1으로 warp 시켜야함. 근데 occlusion에서는 c0을 backward warp를 시킴.. 왜?
...
if is_training:
    for epoch in range(0,maxepoch):
        ...
        for id in np.random.permutation(77782):
            st=time.time()
            color_image=np.float32(cv2.imread("%s/%08d.jpg"%(imgs_dir, id+1)))/255.0
