...
# tensorflow 2.0 version으로 변환했으면 comment 제거
...
def bilinear_up_and_concat(x1, x2, output_channels, in_channels, scope):
    with tf.variable_scope(scope):
        ...
        upconv = tf.layers.Conv2D(upconv,output_channels,[3,3], activation=None,kernel_initializer=tf.contrib.layers.xavier_initializer(), name='up_conv1')
...
def VCN(input, channel=32, output_channel=3,reuse=False,ext=""):
    ...
    conv9=tf.layers.conv2d(conv9,output_channel*4,[3,3],padding='same', activation=None, kernel_initializer=tf.contrib.layers.xavier_initializer(), name=ext+'g_conv9_2')
    return conv9
